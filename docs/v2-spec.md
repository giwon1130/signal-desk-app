...
- [ ] **다크모드 default 여부** — 다크 기본 vs 시스템 따름 (현재 시스템 따름)
- [ ] **추천 종목 시드 정확한 리스트** — 프로필별 6~8개
- [ ] **온보딩 스킵 옵션** — "나중에 설정" 허용 vs 시장 선택 필수
- [ ] **기존 사용자에게 v1 레이아웃 토글 제공 여부** — 거부감 줄이는 안전망, but 유지보수 비용
- [ ] **차트 라이브러리** — CandleVolumeChart 그대로 사용 vs Victory Native 등 도입
- [ ] **종목 정렬/필터** — "종목" 탭에서 손익 정렬 / 시장별 필터 UI
