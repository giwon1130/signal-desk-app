...
| Phase | 내용 | 분량 |
|---|---|---|
| A | DB(V21) + 도메인 + 리포지토리 (Leader/Follow/Post/Call) | 1d |
| B | 종목 자동 인식 (텍스트 파싱 + searchStocks 매칭) | 1d |
| C | 글 작성/게시 API + 가격 lock + 구독 API | 1.5d |
| D | 피드 + 성과 추적 (수익률 derived) | 1d |
| E | "거봐" 알림 (스케줄러 + 푸시) | 1d |
| F | 앱 리딩 탭 (홈/작성/상세/프로필) | 2.5d |
| G | (V1.1) Gemini 시황 초안 + 리더 통계 | 1.5d |
| H | Beta 검증 | 1~2d |
...
- [ ] **콜 종료 정책**: 리더가 수동 종료 vs 목표 도달 시 자동 HIT 마킹 vs 무기한 추적
- [ ] **"거봐" 알림 임계**: +15% 고정 vs 콜별 targetReturnPct 설정 vs 둘 다
- [ ] **시황 글(종목 0개)도 별도 노출** vs 콜 글과 통합 피드
- [ ] **리더 = 모든 사용자** vs 신청/승인 (V1 은 모두 리더 가능 추천)
- [ ] **하락 콜도 알림** 보낼지 (신뢰 위해 정직 vs 리더 기분)
- [ ] **종목 자동인식 오탐** 처리 — 작성자 확인 필수 vs 자동 등록
- [ ] **투자자문 면책** 수위 — 법적 검토 필요 (리딩=유사 자문 리스크)
