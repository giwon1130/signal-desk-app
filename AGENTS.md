...
React Native (Expo) 모바일 앱 + **웹 앱**(RN-Web). Spring Boot 백엔드(`../signal-desk-api`)와 짝.

플랫폼별 타깃:
- **모바일(iOS/Android)**: Today 탭 중심, 빠른 확인용 컴패니언. TestFlight 베타 운영 중(Build #18, 2026-05-22).
- **웹(desktop)**: 3열 레이어 셸(좌 사이드바 · 메인 · 우 컨텍스트) + 티커 리본 + Cmd+K 팔레트 + AIWorkspace(플레이북/성적표). **현재 라이브 미배포** — 2026-05-26 Railway 서비스 정리, 빌드 소스(`src/web/*`)만 보존.
...
- 구현: `src/hooks/useMarketReminder.ts` + `src/components/ReminderSettingsModal.tsx` (+ `reminder_parts/`)
- 헤더의 종(🔔) 아이콘 → 모달에서 토글
- 평일(월~금) 반복 (`Notifications.SchedulableTriggerInputTypes.WEEKLY` × 5개로 분리 등록 — DAILY 쓰면 토/일도 울려서 X):
  - 🇰🇷 한국장: 09:00 KST
  - 🇺🇸 미국장: EDT 22:30 / EST 23:30 KST 자동 분기
...
- 모달에 시장 선호(`MarketPreferencePicker` KR/US/BOTH) 토글 — `me/alert-preferences` 와 연동, 픽·시그널·이벤트 필터에 반영
...
src/
  api.ts                  # market/workspace 호출 + authedFetch + loadAllData
  api/                    # 도메인별 호출 분리
    auth.ts               # 이메일/소셜 인증 + 토큰 저장
    socialAuth.ts         # Google 네이티브 SDK 훅
    ai.ts                 # ai/picks, ai/signals
    disclosures.ts        # disclosures/recent
    events.ts             # events/upcoming
    insights.ts           # insights/today
    media.ts              # media/summaries/latest
    alertPreferences.ts   # me/alert-preferences + marketPreference 타입
    pushDevice.ts         # push/devices, push/alerts (이력)
  types.ts                # 도메인 타입 barrel (re-export)
  types/                  # 도메인별 타입 (백엔드와 1:1 매칭)
    market.ts             # 지수·세션·차트·뉴스·합성위험도·이벤트·top movers
    ai.ts                 # Gemini insight·picks·hidden signals·추천 로그
    workspace.ts          # watchlist/portfolio/종목 검색·상세
    briefing.ts           # 일일 브리핑 (HomeTab hero)
    media.ts              # 모닝/이브닝 브리프, 뉴스 종합
    system.ts             # API envelope·공시·운세·알림 이력·UI 키
  styles.ts               # 모든 스타일
  theme.tsx               # 라이트/다크 팔레트
  utils.ts, utils/        # 포매팅, haptic, 색상 헬퍼
  hooks/                  # useToast, useLivePrices, useMarketReminder, usePushDeepLink
  components/             # 재사용 (Toast, AuthScreen, CandleVolumeChart, ReminderSettingsModal)
...
- **타입 먼저**: 백엔드 응답 변경 시 `src/types/` 도메인 파일부터 갱신 (`market`/`ai`/`workspace`/`briefing`/`media`/`system`). `src/types.ts` 는 barrel 이라 `from '../types'` import 도 호환. `npx tsc --noEmit` 깨끗해야 commit.
...
**웹은 현재 라이브 미배포** — 2026-05-26 Railway 비용 정리로 `signal-desk-web` 서비스가 삭제됐다.
빌드 소스(`src/web/*`, `Dockerfile.web`)는 보존돼 있으므로 재오픈은 한 번에 가능.

재오픈 시 필요한 것:

```bash
# A) Railway 재배포 (Dockerfile.web = expo export → nginx 멀티스테이지)
railway link                                               # 서비스 재생성 필요
railway up --service signal-desk-web --ci --detach

# B) Vercel / Cloudflare Pages 무료 정적 호스팅
npx expo export -p web                                     # dist/ 빌드 → 정적 업로드
```

재오픈 시 체크:
- 운영 URL을 GCP OAuth Client 의 **Authorized JavaScript origins** 에 다시 등록 (+ `http://localhost:8081`, `http://localhost:19006`). trailing slash 금지.
- 번들명은 content-addressed (`index-<hash>.js`). 배포 검증은 해시 변화로 확인.
...
| 커밋 | 요약 |
|------|------|
| (최근) | `src/api.ts`·`src/types.ts` → `src/api/`·`src/types/` 디렉터리 분리 (types.ts/api.ts 는 barrel) |
| (최근) | 웹 서비스 정리 — 2026-05-26 Railway `signal-desk-web` 삭제, 빌드 소스만 보존 |
| (최근) | EVENING_BRIEF 미장 이브닝 브리프 푸시 + Today 탭 딥링크 |
| (최근) | marketPreference(KR/US/BOTH) 필터 — 픽·시그널·로그·이벤트·차트 시장 자동 정렬 |
